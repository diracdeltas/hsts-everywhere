...
import glob
RULES = glob.glob('./rules/*.xml')

def extract_hosts(rules):
    """Get a set of all the hosts from a list of rules"""
    hosts = list()
    for rule in rules:
        tree = etree.parse(rule)
        hosts.extend([target.attrib["host"] for target in tree.xpath("/ruleset/target")])
    return set(hosts)

def main():
    simple_rules = filter(simple, RULES)
    for host in extract_hosts(simple_rules):
        print host
